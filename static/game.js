...
function updateZombies(dt) {
  ...
  for (const z of zombies) {
    ...
    if (z.alerted) {
      ...
      z.walkPhase += dt * 0.25;
      moveWithCollision(z, (dx / d) * ZOMBIE_SPEED * def.speedMult * dt, (dy / d) * ZOMBIE_SPEED * def.speedMult * dt, isSolidForZombie);
      if (d < 26) {
      ...
      if (dog && !dog.isDowned) {
        ...
        if (dd < 24) {
          dog.hp -= (ZOMBIE_DAMAGE * def.dmgMult) * dt * 0.08;
...
function drawZombies() {
  ...
  for (const z of zombies) {
    ...
    if (now < z.alertPulseUntil) {
      ...
      ctx.fillText(z.kind === "screamer" ? "📢" : "❗", s.x, by - 26 * def.sizeMult);
    }
    ctx.fillStyle = "#000";
